...
import {buildGroupName, GUID} from "../utils/iqHelper";
...
export class IQManager extends EventEmitter{
        ...
        async addMessageToQueue(queueName: string, msg: Message) {
                ...
                if(!isGroupExist) {
                        await this.repoClient.addItemToZQ(queueName, groupName);
                        await this.repoClient.setExpiration(queueName, this.ttl);
                }
        ...
        async getListGroupsFromQueue(queueName: string) {
                ...
                return this.repoClient.getAllItemsFromZQ(queueName);
        }
        ...
        async readAllMessageFromQueue(queueName: string) {
                ...
                const groupList = await this.repoClient.getAllItemsFromZQ(queueName);
        ...
        async popupGroupFromQueue(queueName: string) {
                ...
                const groupName = await this.repoClient.popItemFromZQ(queueName);
                if(!groupName)
                        return;
